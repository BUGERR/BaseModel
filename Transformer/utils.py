...
if __name__ == '__main__':
    ...
    sos = sos.fill_(sos_idx)
    print(sos)

    ended = torch.zeros(batch_size, dtype=torch.bool)
    print(ended)
    next_words = torch.tril(torch.ones(batch_size, batch_size, dtype=torch.bool))
    print(next_words)
    print(ended | next_words[:, -2])

    # top_p
    seq_len = 6
    vocab_size = 10

    dec_output = torch.randn(batch_size, seq_len - 1, vocab_size)
    logits = dec_output[:, -1]
    print(logits.shape)
    ...
    # 计算张量沿指定维度的累积和
    cumulative_probs = torch.cumsum(F.softmax(sorted_logits, dim=-1), dim=-1)
    print(cumulative_probs.shape, cumulative_probs)

    indices_to_remove = cumulative_probs > 0.9
    print(indices_to_remove.shape, indices_to_remove)
